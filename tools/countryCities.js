/**
 * Major cities per country for scout rotation.
 * When scout_countries is set in settings, the pipeline derives its
 * city list from here instead of (or merged with) scout_cities.
 */
...
/** Sorted list of all available country names */
export const ALL_COUNTRIES = Object.keys(COUNTRY_CITIES).sort();

/**
 * Given a list of selected country names, return the merged deduplicated
 * list of cities to use as scout_cities.
 */
export function getCitiesForCountries(countries) {
  const seen = new Set();
  const cities = [];

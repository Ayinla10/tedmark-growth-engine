...
export function OrchestrationCanvas({ nodes }: { nodes: OrchestrationNode[] }) {
  ...
  return (
    // containerType enables cqw units so text and badges scale with the
    // canvas exactly like the SVG ring and the fluid robots/core do.
    <div className="relative w-full" style={{ aspectRatio: `${W} / ${H}`, containerType: "inline-size" }}>
      ...
      >
        <p className="text-amber-300/90 font-semibold" style={{ fontSize: "max(9px, 1.2cqw)", letterSpacing: "0.3em" }}>
          J.A.R.V.I.S CORE
        </p>
        <p className="text-slate-400" style={{ fontSize: "max(8px, 1.05cqw)" }}>
      ...
      {nodes.map((node) => {
        ...
        return (
          ...
          >
            ...
            <p className="font-semibold text-slate-100 mt-1" style={{ fontSize: "max(10px, 1.45cqw)" }}>
              {node.name}
            </p>
            <p className="font-medium flex items-center gap-1.5" style={{ color: statusColor, fontSize: "max(9px, 1.2cqw)" }}>
              <span
                className={`rounded-full ${node.status === "inprogress" ? "animate-pulse" : ""}`}
                style={{ background: statusColor, width: "max(5px, 0.6cqw)", height: "max(5px, 0.6cqw)" }}
              />
              {node.statusLabel}
            </p>
            <p className="text-slate-400 leading-snug mt-0.5 line-clamp-2" style={{ fontSize: "max(8px, 1.1cqw)" }}>
              {node.detail}
            </p>
          </motion.div>
        );
